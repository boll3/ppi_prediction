...
\begin{abstract}
  ...
  Obviously, the quality reached by a learning method depends crucially as well on the amount as on the quality of the datasets used for training.
  ...
  As a result, we identified the random forest approach as leading to the cleanest separation of binders and non-binders.
  
  The outlined procedure resulted in an accuracy far above any other available tool:
  area under curve of 0.94,
  specificity of 0.88
  sensitivity: 0.84,
  and a total accuracy of 0.86.
  These quality estimates were calculated using a challenging dataset, as the comparison with other tools clearly underlines (see Fig. \ref{fig:comparison}).
  
  \tool\  is available online, free for academic users.
  The server allows users to scan their query sequence against several large databases for potential binders.
