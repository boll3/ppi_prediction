...
these lead to a highly accurate and extremely fast high-throughput
method.

Therefore, \tool (\textbf{pr}ediction \textbf{o}f \textbf{m}atching \textbf{p}ar\textbf{t}ners)
...
unwanted bias in different applications. \TODO{(is this the best solution for the server?)} 
...
In our next steps we will extend \tool to predict also the binding of short peptides to protein receptors, specifically to those of the GPCR subclass.
...
Based on our extensive tests, we assume that
\tool will be useful for understanding complex networks of protein-protein interactions,
which provide the basis for a broad range of biological functionalities. 
