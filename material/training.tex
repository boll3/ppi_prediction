...
\hspace{1cm}\begin{tikzpicture}[auto,thick, scale=0.5, transform shape,align=center, node distance = 1.5cm]
  ...
  \node[grey,minimum width=11cm,minimum height=11.5cm,below left=0.5cm and 0 of opt]{ \parbox[b][10.5cm]{4cm}{Positive Dataset}};
  ...
  \node (ip1) [io, below left=1.5cm of pclean] {PDB}
  edge[aro](pclean);
  \node (ip2) [io,right=4cm of ip1] {HPRD}
  edge[aro](pclean);
  \node (ip3) [io,below right=0.8cm of ip1] {DIP}
  edge[aro](pclean);
  \node (ip4) [io,right=1cm of ip3] {KUPS}
  edge[aro](pclean);
