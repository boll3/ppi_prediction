...
\begin{abstract}
  ...
  Obviously, the quality reached by a learning method depends crucially on the amount as well as on the quality of the datasets used for training.
  ...
  The outlined procedure resulted in an area under curve (AUC) of 0.94. 
  This is far above any other available tool: \spps\ \cite{Liu:2012}: 0.77, \tri\ \cite{Perovic:2017}:0.73, \lr\ \cite{Pan:2010}: 0.6.
  These quality estimates were calculated using a challenging dataset, as the comparison with the other tools clearly underlines (see Fig. \ref{fig:comparison}).
  
  \tool\  is available online and free for academic users.
  The server allows users to scan their query sequence against several large databases (27k-XXk) for potential binders (see Fig. \ref{fig:workflow} and \ref{fig:webform}).
  Scanning a query sequence against the human database requires around one minute.
  Running \tool\ for independent queries allows the identification of interaction networks (see Fig. \ref{fig:connect}).
